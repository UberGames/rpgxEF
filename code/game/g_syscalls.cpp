...
std::string trap_GetConfigstring(int num, int buffer_size) {
  ...
  syscall(G_GET_CONFIGSTRING, num, buffer.data(), buffer_size);
  auto end = std::find(buffer.begin(), buffer.end(), '\0');
  auto result = std::string{buffer.begin(), end};
  return result;
}
...
int trap_AAS_PredictClientMovement(void *move,
                                   ...
                                   int stopareanum,
                                   int visualize) {
  return syscall(BOTLIB_AAS_PREDICT_CLIENT_MOVEMENT,
                 move,
                 entnum,
                 origin,
                 presencetype,
...
int trap_BotReplyChat(int chatstate,
                      ...
                      char *var6,
                      char *var7) {
  return syscall(BOTLIB_AI_REPLY_CHAT,
                 chatstate,
                 message,
                 mcontext,
                 vcontext,
...
int trap_BotChooseNBGItem(int goalstate,
  ...
  return syscall(BOTLIB_AI_CHOOSE_NBG_ITEM,
                 goalstate,
                 origin,
                 inventory,
                 travelflags,
