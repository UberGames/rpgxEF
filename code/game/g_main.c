...
static void G_FindTeams(void) {
	...
	for (i = 1, e = g_entities + i; i < level.num_entities; i++, e++) {
		...
		if ((e->flags & FL_TEAMSLAVE) != 0)
			continue;
		if ((e->classname != NULL) && (e->type != ENT_FUNC_DOOR)) {//not a door
			if (Q_stricmp("1", e->team) == 0 || Q_stricmp("2", e->team) == 0) {//is trying to tell us it belongs to the TEAM_RED or TEAM_BLUE
				continue;
			}
		}
		e->teammaster = e;
...
void G_RunFrame(int32_t levelTime) {
	...
	for (i = 0; i<level.num_entities; i++, ent++) {
		...
		if (!ent->client) {
			if ((es->eFlags & EF_ANIM_ONCE) == EF_ANIM_ONCE) {//this must be capped render-side
				es->frame++;
			}
		}
