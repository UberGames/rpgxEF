...
static void Cmd_Give_f(gentity_t *ent) {
  ...
  switch (item->giveType) {
    case TYPE_ALL: targEnt->health = ps->stats[STAT_MAX_HEALTH];
      ...
      break;
    case TYPE_HEALTH: targEnt->health = ps->stats[STAT_MAX_HEALTH];
      break;
    case TYPE_WEAPONS: ps->stats[STAT_WEAPONS] = (1 << WP_NUM_WEAPONS) - 1 - (1 << WP_0);
    ...
    case TYPE_HOLDABLE:
      ...
      break;
    case TYPE_WEAPON: ps->stats[STAT_WEAPONS] ^= (1 << item->giveValue);
...
static void Cmd_Kill_f(gentity_t *ent) {
  ...
  {//can't flood-kill
    trap_SendServerCommand(clientNum,
                           va("cp \"Cannot suicide for %d seconds",
                              (lastKillTime[ent->client->ps.clientNum] - (level.time - 60000)) / 1000));
    return;
  }
...
void BroadcastClassChange(gclient_t *client, pclass_t oldPClass) {
  ...
  if (g_classData[client->sess.sessionClass].message[0])
    trap_SendServerCommand(-1,
                           va("cp \"%.15s" S_COLOR_WHITE "%s\n\"",
                              client->pers.netname,
                              g_classData[client->sess.sessionClass].message));
}
...
qboolean SetTeam(gentity_t *ent, const char *s) {
  ...
  if (g_gametype.integer >= GT_TEAM && !isBot) {
    if (!(!Q_stricmp(s, "spectator") || !Q_stricmp(s, "s") || !Q_stricmp(s, "red") || !Q_stricmp(s, "r")
        || !Q_stricmp(s, "blue") || !Q_stricmp(s, "b"))) {
      return qfalse;
    }
  }
...
pclass_t ValueNameForClass(const char *s) {
...
static void Cmd_Team_f(gentity_t *ent) {
  ...
  if (g_classData[sess->sessionClass].isn00b) {
    Com_sprintf(send,
                sizeof(send),
                "%s ^7 doesn't know when to quit trying to change teams!",
                ent->client->pers.netname);

    for (j = MAX_GENTITIES - 1; j > -1;
         j--) { // GSIO: change j++ to j--, also why the gell 1023 and not MAX_GENTITIES???
  ...
  if (trap_Argc() != 2) {
    switch (oldTeam) {
      case TEAM_BLUE: trap_SendServerCommand(ent - g_entities, "print \"Blue team\n\"");
        break;
      case TEAM_RED: trap_SendServerCommand(ent - g_entities, "print \"Red team\n\"");
        break;
      case TEAM_FREE: trap_SendServerCommand(ent - g_entities, "print \"Free team\n\"");
        break;
      case TEAM_SPECTATOR: trap_SendServerCommand(ent - g_entities, "print \"Spectator team\n\"");
        break;
    }
    return;
  }
  ...
  voted = ent->client->ps.eFlags & EF_VOTED ? qtrue : qfalse;
...
static void Cmd_Class_f(gentity_t *ent) {
  ...
  if ((ent->s.eFlags & EF_DEAD) && (client->fraggerTime != -1) && (client->fraggerTime > level.time)) {
    trap_SendServerCommand(ent - g_entities,
                           "print \"Sorry, you can't directly respawn after having been caught fragging.\n\"");
    return;
  }
  ...
  if (g_classData[sess->sessionClass].isn00b) {
    Com_sprintf(send,
                sizeof(send),
                "%s ^7 doesn't know when to quit trying to change classes from n00bie",
                ent->client->pers.netname);
...
void Cmd_FollowCycle_f(gentity_t *ent, int dir) {
  ...
  if (ent->r.svFlags & SVF_BOT) {//bots can't follow!
    return;
  }
...
static void G_SayTo(gentity_t *ent, gentity_t *other, int mode, int color, const char *name, const char *message) {
  ...
  if ((mode == SAY_CLASS || mode == SAY_ADMIN)
      && (IsAdmin(other) && (other->client->noAdminChat == qtrue || rpg_respectPrivacy.integer > 0))) {
    return;
  }
...
static void G_Say(gentity_t *ent, gentity_t *target, int mode, const char *chatText) {
  ...
  switch (mode) {
    ...
    case SAY_ADMIN:
      ...
      target = NULL;
      break;
    case SAY_INVAL: G_LogPrintf("Invalid During Intermission: %s: %s (%s)\n", entPers->netname, chatText, entPers->ip);
  ...
  if (rpg_allowspmaps.integer != 1) {
    if (!Q_stricmp(arg1, "map") &&
        (!Q_stricmp(arg2, "_brig")
            || !Q_stricmp(arg2, "_holodeck_camelot") || !Q_stricmp(arg2, "_holodeck_firingrange")
            || !Q_stricmp(arg2, "_holodeck_garden") || !Q_stricmp(arg2, "_holodeck_highnoon")
            || !Q_stricmp(arg2, "_holodeck_minigame") || !Q_stricmp(arg2, "_holodeck_proton")
            || !Q_stricmp(arg2, "_holodeck_proton2") || !Q_stricmp(arg2, "_holodeck_temple")
            || !Q_stricmp(arg2, "_holodeck_warlord")
            || !Q_stricmp(arg2, "borg1") || !Q_stricmp(arg2, "borg2") || !Q_stricmp(arg2, "borg3")
            || !Q_stricmp(arg2, "borg4") || !Q_stricmp(arg2, "borg5") || !Q_stricmp(arg2, "borg6")
            || !Q_stricmp(arg2, "dn1") || !Q_stricmp(arg2, "dn2") || !Q_stricmp(arg2, "dn3") || !Q_stricmp(arg2, "dn4")
            || !Q_stricmp(arg2, "dn5") || !Q_stricmp(arg2, "dn6") || !Q_stricmp(arg2, "dn8")
            || !Q_stricmp(arg2, "forge1") || !Q_stricmp(arg2, "forge2") || !Q_stricmp(arg2, "forge3")
            || !Q_stricmp(arg2, "forge4") || !Q_stricmp(arg2, "forge5") || !Q_stricmp(arg2, "forgeboss")
            || !Q_stricmp(arg2, "holodeck")
            || !Q_stricmp(arg2, "scav1") || !Q_stricmp(arg2, "scav2") || !Q_stricmp(arg2, "scav3")
            || !Q_stricmp(arg2, "scav3b") || !Q_stricmp(arg2, "scav4") || !Q_stricmp(arg2, "scav5")
            || !Q_stricmp(arg2, "scavboss")
            ...
            || !Q_stricmp(arg2, "tour/deck04") || !Q_stricmp(arg2, "tour/deck05") || !Q_stricmp(arg2, "tour/deck08")
            || !Q_stricmp(arg2, "tour/deck09") || !Q_stricmp(arg2, "tour/deck10") || !Q_stricmp(arg2, "tour/deck11")
            || !Q_stricmp(arg2, "tour/deck15")
            || !Q_stricmp(arg2, "tutorial")
            || !Q_stricmp(arg2, "voy1") || !Q_stricmp(arg2, "voy13") || !Q_stricmp(arg2, "voy14")
            || !Q_stricmp(arg2, "voy15") || !Q_stricmp(arg2, "voy16") || !Q_stricmp(arg2, "voy17")
            || !Q_stricmp(arg2, "voy2") || !Q_stricmp(arg2, "voy20") || !Q_stricmp(arg2, "voy3")
            || !Q_stricmp(arg2, "voy4") || !Q_stricmp(arg2, "voy5") || !Q_stricmp(arg2, "voy6")
            || !Q_stricmp(arg2, "voy7") || !Q_stricmp(arg2, "voy8") || !Q_stricmp(arg2, "voy9"))) {
...
static void Cmd_ForceClass_f(gentity_t *ent) {
  ...
  Com_sprintf(send,
              sizeof(send),
              "%s ^7put %s into the ^7%s class",
              ent->client->pers.netname,
              target->client->pers.netname,
              ClassForValueName(target->client->sess.sessionClass));
...
static void Cmd_ForceKillRadius_f(gentity_t *ent) {
  ...
  if ((rpg_forcekillradiuswaittime.integer - (level.time - LastFKRadius[clientNum]) > 0)) {
    ...
    trap_SendServerCommand(clientNum,
                           va("cp \"Cannot use Force Kill Radius Command for %d seconds",
                              (int) ((rpg_forcekillradiuswaittime.integer - (level.time - LastFKRadius[clientNum]))
                                  * 0.001))); // GSIO was / 1000
    return;
  }
  ...
  for (i = 0; i < level.numConnectedClients; i++) {
    ...
    G_SayTo(ent,
            ...
            COLOR_CYAN,
            "^7Server: ",
            va("%s has triggred a force kill radius command", ent->client->pers.netname));
...
static void Cmd_TargetKick_f(gentity_t *ent) {
  ...
  for (i = 0; i < level.maxclients; i++) {
    ...
    G_SayTo(ent,
            ...
            COLOR_CYAN,
            "^7Server: ",
            va("%s has kicked %s", ent->client->pers.netname, target->client->pers.netname));
  }
}
...
void DragCheck(void) {
  ...
  for (i = 0; i < MAX_CLIENTS; i++) {
    ...
    tPs->origin[0] += (DragDat[i].distance
        * cos(DEG2RAD(ePs->viewangles[1])));//(ent->client->ps.viewangles[1] * 0.017453292222222222222222222222222 ) );
    tPs->origin[1] += (DragDat[i].distance
        * sin(DEG2RAD(ePs->viewangles[1])));//(ent->client->ps.viewangles[1] * 0.017453292222222222222222222222222) );
...
static void Cmd_Rank_f(gentity_t *ent) {
  ...
  if (!ArgStr[0]) { //If no arguments (ie player wants to see current rank)
    ...
    Com_sprintf(tmpScore,
                sizeof(tmpScore),
                "%s - %s",
                g_rankNames[OldScore].formalName,
                g_rankNames[OldScore].consoleName); //Put OldScore in a string variable
    ...
    if (!Q_stricmp(ArgStr, g_rankNames[i].consoleName)) {
      ...
      if (!MaxRankHit) {
        ...
        if (!MaxRankHit)
          ...
          trap_SendServerCommand(ent - g_entities,
                                 va("print \"You cannot set your rank that high on this server.\n\""));
  ...
  if (OldScore > ent->client->ps.persistant[PERS_SCORE]) {
    trap_SendServerCommand(-1,
                           va("print \"%s" S_COLOR_WHITE " was demoted to %s\n\"",
                              ent->client->pers.netname,
                              g_rankNames[i].formalName));
  } else {
    trap_SendServerCommand(-1,
                           va("print \"%s" S_COLOR_WHITE " was promoted to %s\n\"",
                              ent->client->pers.netname,
                              g_rankNames[i].formalName));
  }
}
...
static void Cmd_AdminLogin_f(gentity_t *ent) {
  ...
  for (j = 0; g_classData[j].consoleName[0] && j < MAX_CLASSES; j++) {
    ...
    if (g_classData[j].isAdmin && !Q_stricmp(classPass, arg)) {
      if (IsAdmin(ent) == qfalse) {
        ...
        trap_SendServerCommand(ent - g_entities,
                               va("print \"You are already logged in as an admin or in the admin class.\n\""));
        return;
      }
      break;
    }
  ...
  if (ent->client->AdminFailed == 3) {
    ...
    Com_sprintf(send,
                sizeof(send),
                "%s tried to login as an admin, failed 3 times and so was kicked.\n",
                ent->client->pers.netname);
    ...
    trap_SendServerCommand(ent - g_entities,
                           va("print \"You have entered an incorrect password, if you enter a wrong password %i more times you will be kicked.\n\"",
                              (3 - ent->client->AdminFailed)));

    //Send warning to other admins
    Com_sprintf(send,
                sizeof(send),
                "%s tried to login as an admin and got an incorrect password, %i tries left.\n",
                ent->client->pers.netname,
                (3 - ent->client->AdminFailed));
    for (j = level.maxclients - 1; j > -1; j--) { // changed j++ for into j--
      sayA = &g_entities[j];
...
static void Cmd_Revive_f(gentity_t *ent) {
  ...
  if (!pla_str[0] && (ent && ent->client) && !(ent->health <= 1)
      && !(ent->client->ps.pm_type == PM_DEAD)) { //if user added no args (ie wanted the parameters)
    trap_SendServerCommand(ent - g_entities,
...
static void Cmd_n00b_f(gentity_t *ent) {
  ...
  for (i = 0; g_classData[i].consoleName[0] && i < MAX_CLASSES; i++) {
    if (g_classData[i].isn00b) {
      ...
      Q_strncpyz(target->client->origClass,
                 ClassNameForValue(target->client->sess.sessionClass),
                 sizeof(target->client->origClass));
...
static void Cmd_EntList_f(gentity_t *ent) {
  ...
  for (i = 0, mapEnt = g_entities;
    ...
    if ((!Q_stricmpn(mapEnt->classname, "fx_", 3))
        || (!Q_stricmp(mapEnt->classname, "func_usable") && ent->targetname)) {
      if (mapEnt->use) {
        memset(&entBuffer, 0, sizeof(entBuffer));

        if (mapEnt->targetname) {
          Com_sprintf(entBuffer,
                      sizeof(entBuffer),
                      "ClassName: '%s', TargetName: '%s', ID: %i\n",
                      mapEnt->classname,
                      mapEnt->targetname,
                      i);
        } else {
  ...
  if (targEnt /*&& TransDat[beamTarget->client->ps.clientNum].beamTime == 0*/ ) {
    ...
    if (TransDat[beamTarget->client->ps.clientNum].beamTime == 0) {
      ...
      if (beamTarget->client->ps.clientNum == ent->client->ps.clientNum) {
        trap_SendServerCommand(ent - g_entities,
                               va("chat \"Unable to comply. You are already within a transport cycle.\n\" "));
      } else {
        trap_SendServerCommand(ent - g_entities,
                               va("chat \"Unable to comply. Subject is already within a transport cycle.\n\" "));
      }
    }
  } else {
...
static void Cmd_ForcePlayer_f(gentity_t *ent) {
  ...
  Com_sprintf(send,
              sizeof(send),
              "%s ^7changed %s's %s setting to ^7to %s",
              pers->netname,
              other->client->pers.netname,
              str,
              str2);
...
static void Cmd_BeamToPlayer_f(gentity_t *ent) {
  ...
  if (trap_Argc() == 2) {
    ...
    if (Q_stricmp(argStr, "all") == 0) {
      trap_SendServerCommand(ent - g_entities,
                             "print \"ERROR. You can not beam a player to everyone.\nPerhaps you meant /beamToPlayer all <number>.\n\" ");
      return;
    }
  ...
  do {
    if (1) { // sorry
      ...
      startPoint = irandom(1,
                           7); //we'll randomize where it checks, so players won't potentially beam to the same spot each time.
      ...
      if (TransDat[beamee->client->ps.clientNum].beamTime == 0) {
        if (j == ent->client->ps.clientNum) {
          trap_SendServerCommand(ent - g_entities,
                                 va("chat \"Initiating transport to player %s^7's co-ordinates.\n\" ",
                                    target->client->pers.netname));
        } else {
          trap_SendServerCommand(ent - g_entities,
                                 va("chat \"Transporting %s^7 to player %s^7's co-ordinates.\n\" ",
                                    beamee->client->pers.netname,
                                    target->client->pers.netname));
        }
        //commence beaming
...
static void Cmd_DoEmote_f(gentity_t *ent) {
  ...
  if (!Q_stricmp(argStr, "eyes_shut") || !Q_stricmp(argStr, "eyes_frown") || !Q_stricmpn(argStr, "alert2", 6)
      || !Q_stricmpn(argStr, "alert", 5)) {
...
static void Cmd_fxGun_f(gentity_t *ent) {
  ...
  if (!Q_stricmp(arg, "default")) {
    ...
    if (!arg[0]) {
      trap_SendServerCommand(ent - g_entities,
                             "print \"Syntax: /fxGun sparks <spark time interval> <time length of effect> | in milliseconds\n\" ");
      return;
    }
    ...
    if (!arg[0]) {
      trap_SendServerCommand(ent - g_entities,
                             "print \"Syntax: /fxGun drips <type of drips: 0,1,2> <drips intensity: 0.0->1.0> <time length of effect: 1000 = 1 second> \n\" ");
      return;
    }
    ...
    if (!arg[0]) {
      trap_SendServerCommand(ent - g_entities,
                             "print \"Syntax: /fxGun smoke <smoke radius size> <time length of effect: 1000 = 1 second> \n\" ");
      return;
    }
    ...
    if (!arg[0]) {
      trap_SendServerCommand(ent - g_entities,
                             "print \"Syntax: /fxGun surf_explosion <explosion radius> <camera shake intensity> \n\" ");
      return;
    }
...
static void Cmd_Kick2_f(gentity_t *ent) {
  ...
  if (!str[0]) {
    trap_SendServerCommand(ent - g_entities,
                           "print \"Usage: Kicks specified user with a reason\n\nCommand: kick [User ID] <Reason>\n\"");
    return;
  }
...
static void Cmd_ClampInfo_f(gentity_t *ent) {
  ...
  if (!buffer[0]) {
    trap_SendServerCommand(ent - g_entities,
                           "print \"Usage: Force stops players from changing their user info\n\nCommand: clampInfo <client ID>\n\"");
    return;
  }
...
static void Cmd_MeActionLocal_f(gentity_t *ent) {
  ...
  if (trap_Argc() < 2) {
    trap_SendServerCommand(ent - g_entities,
                           "print \"Chat Emote Command in your Immediate Area.\nUsage: type a 3rd person action sentence.\nExample: 'sits down'.\n\"");
    return;
  }
  ...
  for (i = 0; i < level.maxclients; i++) {
    ...
    if (!OtherPlayer->client->noAdminChat && IsAdmin(OtherPlayer))
      trap_SendServerCommand(-1,
                             va("print \"%s" S_COLOR_CYAN" (locally) " S_COLOR_WHITE "%s\n\"",
                                ent->client->pers.netname,
                                message));
...
static void Cmd_changeFreq(gentity_t *ent) {
  ...
  if (trap_Argc() < 2) {
    G_PrintfClient(ent,
                   "Usage: changeFreq value, -1 = level 10 forcefield, 0 = random or any value between 0 and 20\n");
    return;
  }
  trap_Argv(1, arg, sizeof(arg));
  i = atof(arg);
...
static void Cmd_zonelist_f(gentity_t *ent) {
  ...
  if (type == 0 || type > 2) {
    trap_SendServerCommand(ent - g_entities,
                           va("print \"Usage: zonelist <type> where type is one of the following types of zones:\n--1 = safezones for Selfdestruct\n--2 = MSD-HUD zones\n\" "));
    return;
  }
  ...
  if (!Q_stricmp(arg, "start")) {
    ...
    while ((safezone = G_Find(safezone, FOFS(classname), "target_zone")) != NULL) {
      if (!destructEnt->bluename && safezone->count == 1 && safezone->spawnflags & 2) {
        G_PrintfClient(ent,
                       "^1ERROR: safezone must be given for maps consisting of multiple ships/stations (like rpg_runabout). For a list of safezones use this command: zonelist 1 . Removing entity.");
    ...
    ETAsec = floor(modf(((floor(destructEnt->damage / 1000) - floor(level.time / 1000)) / 60), &ETAmin)
                       * 60); //break it apart, put off the minutes and return the floored secs
    ...
    G_PrintfClient(ent,
                   "audio: set this 0 if you do not want to display the countdown-clock in the top center of your screen, else set this 1.");
    G_PrintfClient(ent,
                   "safezone: Only for maps with multiple ships (like rpg_runabout). it will not be used for processing. Set NULL to skip.");
    G_PrintfClient(ent, "target: Optional Argument for Effects to fire once the countdown hist 0.");
    G_PrintfClient(ent,
                   "The entity will automatically shake everyones screen and kill all clients outside an active target_zone configured as safezone.");
...
static void Cmd_shipdamage_f(gentity_t *ent) {
  ...
  if (!healthEnt) {//specific search did not turn up any results so fall back to the first entity
    trap_SendServerCommand(ent - g_entities,
                           va("print \"^3 Warning: no healthEnt with targetname '%s' found, falling back to first entity found.\n\"",
                              target));
    ...
    if (G_Find(healthEnt, FOFS(classname), "target_shiphealth")) {//abort if we do have more than one healthEnt
      trap_SendServerCommand(ent - g_entities,
                             va("print \"^1 ERROR: This map has more than one target_shiphealth, therefore a target needs to be specified. Aborting call.\n\""));
      return;
    }
  }
...
static void Cmd_shiphealth_f(gentity_t *ent) {
  ...
  while ((healthEnt = G_Find(healthEnt, FOFS(classname), "target_shiphealth"))
      != NULL) { //do this for every healthEnt you find
    ...
    if (CHS == 0) {
      ...
      switch (SI) {
        case -2: trap_SendServerCommand(ent - g_entities, "print \"^1 Shields are offline\n\"");
          break;
        case -1: trap_SendServerCommand(ent - g_entities, "print \"^1 Shields are inoperable\n\"");
          break;
        case 0: trap_SendServerCommand(ent - g_entities, "print \"^3 Shields are standing by\n\"");
          break;
        case 1: trap_SendServerCommand(ent - g_entities, "print \"^2 Shields are online\n\"");
          break;
      }
      if (CSS > 0)
        trap_SendServerCommand(ent - g_entities,
                               va("print \"^%i Shield Capactiy at %.0f Percent (%i of %i Points)\n\"",
                                  SCI,
                                  RSS,
                                  CSS,
                                  TSS));
      trap_SendServerCommand(ent - g_entities,
                             va("print \"^%i Structual Integrity at %.0f Percent (%i of %i Points)\n\n\"",
                                HCI,
                                RHS,
                                CHS,
                                THS));
...
static void Cmd_reloadtorpedos_f(gentity_t *ent) {
  ...
  if (atoi(arg) == 0 || !Q_stricmp(target, "")) {
    G_PrintfClient(ent,
                   "Usage: reloadtorpedos [amount] [target] where amount is the number of torpedos the launcher will have after the command executed.");
    G_PrintfClient(ent,
                   "Set -2 to depleet, set -1 to max out. You can not load more torpedos than the entity was designed for. If you do it will just Max out.");
    G_PrintfClient(ent, "target is the specific targetname of the fx_torpedo that shall be reloaded.");
    G_PrintfClient(ent,
                   "If set to all every limited launcher on the map will be set to amount or maxed out accordingly.");
...
static void Cmd_getEntInfo_f(gentity_t *ent) {
  ...
  G_PrintfClient(ent,
                 "Classname: %s\n Targetname: %s\n Target: %s\n Spawnflags: %i\n Bmodel: %s\n\"",
                 g_entities[i].classname,
                 g_entities[i].targetname,
                 g_entities[i].target,
                 g_entities[i].spawnflags,
                 g_entities[i].model);
}
...
static void Cmd_spawnTentity_f(gentity_t *ent) {
  ...
  if (!newEnt) {
    trap_SendServerCommand(clientNum,
                           "print \"Could not spawn new entity. Most likely there are no free entities left.\n\"");
    return;
  }
  ...
  if (!Q_stricmpn(arg, "trigger_", 8)) {
    if (!Q_stricmp(arg, "trigger_teleport")) {
      ...
      if (numArgs < 5) {
        ...
        trap_SendServerCommand(clientNum,
                               "print \"Usage: spawnTEnt trigger_teleport sizeX sizeY sizeZ target (spawnflags swapname speed)\n\"");
        return;
      }
      trap_Argv(2, arg, sizeof(arg));
      arg2 = arg;
      ...
      if (numArgs < 6) {
        ...
        trap_SendServerCommand(clientNum,
                               "print \"Usage: spawnTEnt trigger_multiple sizeX sizeY sizeZ target wait\n\"");
        return;
      }
      trap_Argv(2, arg, sizeof(arg));
      arg2 = arg;
    ...
    if (!Q_stricmp(arg, "fx_spark")) {
      ...
      if (numArgs < 2) {
        ...
        trap_SendServerCommand(clientNum,
                               "print \"Usage: spawnTEnt fx_surface_explosion targetname (spawnflags target damage radius speed)\"\n");
        return;
      }
      trap_Argv(2, arg, sizeof(arg));
      newEnt->targetname = G_NewString(arg);
      ...
      if (numArgs < 3) {
        ...
        trap_SendServerCommand(clientNum,
                               "print \"Usage: spawnTEnt fx_blow_chunks targetname target (radius material)\n\"");
        return;
      }
      trap_Argv(2, arg, sizeof(arg));
      newEnt->targetname = G_NewString(arg);
      ...
      if (numArgs < 2) {
        ...
        trap_SendServerCommand(clientNum,
                               "print \"Usage: spawnTEnt fx_electrical_explosion targetname (target radius damage)\n\"");
        return;
      }
      trap_Argv(2, arg, sizeof(arg));
      newEnt->targetname = G_NewString(arg);
    ...
    if (!Q_stricmp(arg, "target_boolean")) {
      ...
      if (numArgs < 3) {
        ...
        trap_SendServerCommand(clientNum,
                               "print \"Usage spawnTEnt target_doorlock target targetname (spawnflags lockMsg unlockMsg)\n\"");
        return;
      }
      trap_Argv(2, arg, sizeof(arg));
      newEnt->target = G_NewString(arg);
      ...
      if (numArgs < 3) {
        ...
        trap_SendServerCommand(clientNum,
                               "print \"Usage: spawnTEnt target_delay targetname target delay (spawnflags random)\n\"");
        return;
      }
      trap_Argv(2, arg, sizeof(arg));
      newEnt->targetname = G_NewString(arg);
      ...
      if (numArgs < 3) {
        ...
        trap_SendServerCommand(clientNum,
                               "print \"Usage: spawnTEnt target_give targetname items (example for items: WP_5 | WP_14)\n\"");
        return;
      }
      trap_Argv(2, arg, sizeof(arg));
      newEnt->targetname = G_NewString(arg);
      ...
      if (numArgs < 3) {
        ...
        trap_SendServerCommand(clientNum,
                               "print \"Usage: spawnTEnt target_gravity targetname gravity (spawnflags)\n\"");
        return;
      }
      trap_Argv(2, arg, sizeof(arg));
      newEnt->targetname = G_NewString(arg);
      ...
      if (numArgs < 3) {
        ...
        trap_SendServerCommand(clientNum,
                               "print \"Usage: spawnTEnt target_teleporter targetname target (spawnflags swapname)\n\"");
        return;
      }
      trap_Argv(2, arg, sizeof(arg));
      newEnt->targetname = G_NewString(arg);
      ...
      if (numArgs < 3) {
        ...
        trap_SendServerCommand(clientNum,
                               "print \"Usage: spawnTEnt target_speaker targetname sound (spawnflags wait random)\n\"");
        return;
      }
      trap_Argv(2, arg, sizeof(arg));
      newEnt->targetname = G_NewString(arg);
  ...
  if (newEnt) {
    ...
    trap_SendServerCommand(clientNum,
                           va("print \"Spawned entity of type %s with entity number %i.\"",
                              newEnt->classname,
                              newEnt->s.number));
  }
}
