...
struct gentity_s {
  ...
  int
      clipmask;            //!< brushes with this content value will be collided again when moving. items and corpses do not collide against players, for instance
  ...
  const char *team;                //!< for teamchains
  ...
  void (*use)(gentity_t *self,
              gentity_t *other,
              gentity_t *activator); //!< function that gets called if the entity is used
  ...
  void (*die)(gentity_t *self,
              ...
              gentity_t *attacker,
              int damage,
              int mod); //!< function that gets called if entity dies
...
struct gclient_s {
  ...
  int
      lastCmdTime;        //!< level.time of last usercmd_t, for EF_CONNECTION we can't just use pers.lastCommand.time, because of the g_sycronousclients case
  int buttons;
  int oldbuttons;
  int latched_buttons;
...
typedef struct {
  ...
  // intermission state
  int
      intermissionQueued;        /*!< Intermission was qualified, but wait INTERMISSION_DELAY_TIME before actually going there so the last frag can be watched.  Disable future kills during this delay */
  ...
  //RPG-X - Decoy index
  int
      decoyIndex;                /*!< 0-128. Counting decoys.  If we do hit the end (omfg), start from the beginning again lol */
  ...
  int g_entScannables[MAX_ENTSCANNABLES][2
  ];    /*!< first cell is the entity ID, the second is the id of the object in question */
  ...
  int
      overrideCalcLiftTravelDuration;            /*!< Indicated whether the level author wants to override rpg_calcLiftTravelDuration */
} level_locals_t;
 ...
 */
void G_Weapon_CalcMuzzlePoint(gentity_t *ent,
                              vec3_t forward,
                              vec3_t right,
                              vec3_t up,
                              vec3_t muzzlePoint,
...
typedef struct {
  //TiM :
  int
      beamTime;                //!< Time the beam was initialized. 4 seconds after this, the player will teleport, 8 seconds later the effect will reset

  transVecData_t currentCoord;            //!< If they are beaming, have the active variables placed here
  transVecData_t
      storedCoord[TPT_MAX];    //!< Any separate beam data (ie, tricorder, portable beam etc) is kept here till needed.

  int LastClick;                //!< Last click (in level time) when feature was used (used to prevent accidents)
  qboolean
      beamed;                    //!< Once we've called the teleport, make this true.  Or else we'll teleport on every clock cycle >.<
} RPGX_SiteTOSiteData;
...
std::string trap_GetConfigstring(int num, int buffer_size);
