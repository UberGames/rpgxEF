...
void SP_target_speaker(gentity_t *ent) {
  ...
  if (!G_SpawnString("noise", "NOSOUND", &s)
      && !ent->count) { // if ent->count then it is a spawned sound, either by spawnEnt or *.spawn
...
void target_shake_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
  trap_SetConfigstring(CS_CAMERA_SHAKE,
                       va("%f %i",
                          self->distance/*was self->intensity*/,
                          ((int) (level.time - level.startTime) + (int) (self->wait * 1000))));
}
...
extern void BG_LanguageFilename(const char *baseName, const char *baseExtension, const char *finalName);
...
void target_warp_use(gentity_t *ent, gentity_t *other, gentity_t *activator) {
  ...
  if (!Q_stricmp(activator->target, ent->truename)) {
    ...
    for (i = 0; i < MAX_GENTITIES; i++) {
      ...
      } else if (!Q_stricmp(g_entities[i].classname, "func_train")
          && !Q_stricmp(g_entities[i].swapname, ent->bluename)) {
        target = &g_entities[i];
        if (!target) continue;
        if (target->count == 1) {
          target->s.solid = 0;
...
void target_selfdestruct_think(gentity_t *ent) {
  ...
  if (healthEnt && G_Find(healthEnt, FOFS(classname), "target_shiphealth") == NULL) {
    healthEnt->damage = healthEnt->health
        + healthEnt->splashRadius; //let's use the healthent killfunc if we have just one. makes a lot of stuff easier.
    ...
    while ((client = G_Find(client, FOFS(classname), "player")) != NULL) {
      if (client->client->nokilli != 1)
        G_Damage(client,
                 ...
                 999999,
                 0,
                 MOD_TRIGGER_HURT); //maybe a new message ala "[Charname] did not abandon ship."
    }
...
void SP_target_selfdestruct(gentity_t *ent) {
  ...
  else if (ETAsec / 10 < 1) //get leading 0 for secs
    trap_SendServerCommand(-1,
                           va("servermsg \"^1Self Destruct in %.0f:0%.0f; There will be no ^1further audio warnings.\"",
                              ETAmin,
                              ETAsec));
  else
    trap_SendServerCommand(-1,
                           va("servermsg \"^1Self Destruct in %.0f:%.0f; There will be no ^1further audio warnings.\"",
                              ETAmin,
                              ETAsec));
  ent->r.svFlags |= SVF_BROADCAST;
  trap_LinkEntity(ent);
    ...
    if (msdzone->count == 2 && Q_stricmp(msdzone->targetname, ent->paintarget)) {
      ...
      for (n = 0; n < num; n++) {
        if (entlist[n] < g_maxclients.integer && g_entities[entlist[n]].client) {
          while ((client = G_Find(client, FOFS(classname), "player")) != NULL) {
            if (client->s.number == entlist[n]) {
              trap_SendServerCommand(client->s.number,
                                     va("shiphealthupdate %.0f %.0f %i ",
                                        floor(ent->count / ent->health),
                                        floor(ent->n00bCount / ent->splashRadius),
                                        ent->splashDamage));
