...
void QDECL Com_sprintf(const char *dest, int size, const char *fmt, ...) {
  int len;
  va_list argptr;
  char *bigbuffer;
...
std::string Info_ValueForKey(const std::string &s, const std::string &key) {
  char pkey[BIG_INFO_KEY];
  static char value[2][BIG_INFO_VALUE];    /* use two buffers so compares
											work without stomping on each other */
  static int valueindex = 0;
  ...
  while (true) {
    ...
    *o = 0;
    ...
    while (*s_it != '\\' && s_it != s.end()) {
      *o++ = *s_it++;
    }
    *o = 0;

    if (!Q_stricmp(key.data(), pkey)) {
      return value[valueindex];
    }
