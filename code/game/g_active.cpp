...
static void TryUse(gentity_t *ent) {
  ...
  if (target && target->use && Q_stricmp("func_usable", target->classname) == 0) {//usable brush
    if (target->team && atoi(target->team) != 0) {//usable has a team
      if (atoi(target->team) != sess->sessionTeam) {//not on my team
        //TiM - return
        return;
      }
    }
...
#endif
    return;
  } else if (target && target->use && Q_stricmp("misc_ammo_station", target->classname) == 0) {//ammo station
    if (sess->sessionTeam) {
      if (target->team) {
...
#endif
    return;
  } else if ((target && target->s.number == ENTITYNUM_WORLD)
      || (target->s.pos.trType == TR_STATIONARY && !(trace.surfaceFlags & SURF_NOIMPACT) && !target->takedamage)) {
    return;
  }
  //FIXME: Play a failure sound
}
...
===============
*/
static void G_SetClientSound(gentity_t *ent) {    // 3/28/00 kef -- this is dumb.
...
*	\author Ubergames
*/
static const char *TimedMessage(void) {
  char *message;
  container_p c;
...
static void ClientTimerActions(gentity_t *ent, int msec) {
  ...
  if (rpg_timedmessagetime.value) {
    ...
    if (level.time > (level.message + (messageTime * 60000))) {
      ...
      message =
          TimedMessage(); //Since we're working with a gloabl scope variable, there's no need for this thing to have parameters:)
  ...
  while (client->timeResidual >= 1000) {
    ...
    if (!client->ps.stats[STAT_HOLDABLE_ITEM]) {//holding nothing...
      if (client->ps.stats[STAT_USEABLE_PLACED] > 0) {//we're in some kind of countdown
        //so count down
...
void ShieldGoSolid(gentity_t *self) {
  ...
  trap_Trace(&tr,
             ...
             self->s.number,
             CONTENTS_BODY);
  if (tr.startsolid) {    // gah, we can't activate yet
    self->nextthink = level.time + 200;
    self->think = ShieldGoSolid;
    trap_LinkEntity(self);
  } else { // get hard... huh-huh...
    self->r.contents = CONTENTS_SOLID;
    self->s.eFlags &= ~(EF_ITEMPLACEHOLDER);
    self->nextthink = level.time + 1000;
    self->think = ShieldThink;
    self->takedamage =
        qtrue;//RPG-X: - RedTechie use to be qtrue //TiM - made true again. should be okay so long as the health isn't decremented
...
void ShieldTouch(gentity_t *self, gentity_t *other, trace_t *trace) {
  ...
  if (IsAdmin(other) || (rpg_borgAdapt.integer && rpg_borgMoveThroughFields.integer
      && IsBorg(other))/*other->client->sess.sessionClass == PC_ADMIN*/ ) {
...
void CreateShield(gentity_t *ent) {
  ...
  if (tr.startsolid) {    // Something in the way!
    // make the shield non-solid very briefly
...
static qboolean PlaceShield(gentity_t *playerent) {
  ...
  if (shieldAttachSound == 0) {
    ...
    shieldMurderSound =
        G_SoundIndex("sound/movers/forcefield/forcefieldtouch.wav"); //RPG-X: - RedTechie Added shild ZAP! sound //sound/world/electro.wav
  ...
  if (tr.fraction > 0.9) {//room in front
    ...
    if (!tr.startsolid && !tr.allsolid) {
      ...
      if (fabs(fwd[0]) > fabs(fwd[1])) {    // shield is north/south, facing east.
        shield->s.angles[YAW] = 0;
      } else {    // shield is along the east/west axis, facing north
...
static void ClientEvents(gentity_t *ent, int oldEventSequence) {
  ...
  for (i = oldEventSequence; i < ps->eventSequence; i++) {
    ...
    switch (event) {
      ...
      case EV_FIRE_WEAPON: FireWeapon(ent, qfalse);
        break;

      case EV_ALT_FIRE: FireWeapon(ent, qtrue);
        break;

      case EV_FIRE_EMPTY_PHASER: FireWeapon(ent, qfalse);
        break;
      ...
      case EV_USE_ITEM1:        // transporter
        ...
        if (TransDat[ps->clientNum].beamTime == 0 &&
          ...
          trap_SendServerCommand(ent - g_entities,
                                 va("chat \"Site to Site Transporter Location Confirmed.\nPress again to Energize.\""));
        ...
        if (TransDat[ps->clientNum].beamTime == 0 && level.time > ps->powerups[PW_QUAD]) {
          ...
          memset(&TransDat[ps->clientNum].storedCoord[TPT_PORTABLE],
                 0,
                 sizeof(TransDat[ps->clientNum].storedCoord[TPT_PORTABLE]));
        } else {
      ...
      case EV_USE_ITEM2:        // medkit
        ...
        if (ent->health < ps->stats[STAT_MAX_HEALTH]) {    // If that doesn't bring us up to 100, make it go up to 100.
          ent->health = ps->stats[STAT_MAX_HEALTH];
        } else if (ent->health > ps->stats[STAT_MAX_HEALTH]
            * 2) {    // Otherwise, 25 is all you get.  Just make sure we don't go above 200.
      ...
      case EV_USE_ITEM5:        // decoy
        if (!PlaceDecoy(ent)) {//couldn't place it
...
void ThrowWeapon(gentity_t *ent, char *txt) {
  ...
  item = BG_FindItemForWeapon(static_cast<weapon_t>(ps->weapon));
...
static void ClientThink_real(gentity_t *ent) {
  ...
  if (ps->powerups[PW_HASTE]) {
    ps->speed *= 1.5;
  } else if (ps->powerups[PW_FLIGHT]) {//flying around
    ps->speed *= 1.3;
  } else if (ps->stats[STAT_HEALTH] <= 20) {
    ps->speed *= 0.55;
  }

  if ((ps->powerups[PW_EVOSUIT]) && (ps->gravity == 0)) {//Evosuit time.. RPG-X | Phenix | 8/8/2004
    ps->speed *= 1.3;
  }
