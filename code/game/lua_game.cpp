...
static int Game_AlertSetup(lua_State *L) {
  ...
  luaAlertState->mode = luaL_checkinteger(L, 9);
...
static int Game_AlertAddShader(lua_State *L) {
  ...
  if (luaAlertState->shaders[cond] == NULL) {
    ...
    luaAlertState->shaders[cond] = static_cast<char *>(tmp);
    Com_sprintf(luaAlertState->shaders[cond],
                sizeof(luaAlertState->shaders[cond]),
                "%s\n%s",
                luaAlertState->shaders[cond],
                shader);
  }
