...
int G_ParseInfos(char *buf, int max, char *infos[]) {
  ...
  while (1) {
    ...
    infos[count] =
        static_cast<char *>(G_Alloc(strlen(info) + strlen("\\num\\") + strlen(va("%d", MAX_ARENAS)) + 1));
    if (infos[count]) {
...
/*	if ( skill >= 1 && skill < 2 ) {
  ...
  if (bot->client->sess.sessionTeam != preTeam) {
    ...
    if (bot->client->sess.sessionTeam == TEAM_SPECTATOR) {
      bot->client->sess.sessionTeam = static_cast<team_t>(preTeam);
    }
...
void G_InitBots(qboolean restart) {
  ...
  if (g_gametype.integer == GT_SINGLE_PLAYER) {
    ...
    if (g_holoIntro.integer) {    // The player will be looking at the holodeck doors for the first five seconds, so take that into account.
