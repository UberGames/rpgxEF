...
qboolean EntityIsInvisible(aas_entityinfo_t *entinfo) {
  if (entinfo->powerups & (1 << PW_GHOST)) {    // 50% chance of being visible?
    if (((unsigned int) (level.time) / 1024)
        & 0x01)        // Every second or so, the bot will see the player, so he doesn't jitter.
    {
      return qtrue;
    } else {
      return qfalse;
...
qboolean BotIsDead(bot_state_t *bs) {
  return (bs->cur_ps.pm_type == PM_DEAD) ? qtrue : qfalse;
}
...
qboolean BotIntermission(bot_state_t *bs) {
  ...
  return (bs->cur_ps.pm_type == PM_FREEZE || bs->cur_ps.pm_type == PM_INTERMISSION) ? qtrue : qfalse;
}
...
qboolean BotInLavaOrSlime(bot_state_t *bs) {
  ...
  feet[2] -= 23;
  return (trap_AAS_PointContents(feet) & (CONTENTS_LAVA | CONTENTS_SLIME)) ? qtrue : qfalse;
}
...
float BotAggression(bot_state_t *bs) {
  ...
  if (bs->inventory[INVENTORY_DREADNOUGHT] > 0 &&
      bs->inventory[INVENTORY_DREADNOUGHTAMMO] > 0)
    return 100;

  if (bs->inventory[INVENTORY_TETRION] > 0 &&
      bs->inventory[INVENTORY_TETRIONAMMO] > 0)
    return 95;

  if (bs->inventory[INVENTORY_QUANTUM] > 0 &&
      bs->inventory[INVENTORY_QUANTUMAMMO] > 0)
    return 90;

  if (bs->inventory[INVENTORY_STASIS] > 0 &&
      bs->inventory[INVENTORY_STASISAMMO] > 0)
    return 85;

  if (bs->inventory[INVENTORY_SCAVENGER] > 0 &&
      bs->inventory[INVENTORY_SCAVENGERAMMO] > 0)
    return 80;
  ...
  if (bs->inventory[INVENTORY_IMOD] > 0 &&
      bs->inventory[INVENTORY_IMODAMMO] > 0)
    return 70;

  if (bs->inventory[INVENTORY_COMPRESSION] > 0 &&
      bs->inventory[INVENTORY_COMPRESSIONAMMO] > 0)
    return 65;
...
int BotCanAndWantsToRocketJump(bot_state_t *bs) {
  ...
  //if low on health
  if (bs->inventory[INVENTORY_HEALTH] < 50) {    //if not full health
    if (rpg_selfdamage.integer != 0) {
      return qfalse;
    }
  }
  if (bs->inventory[INVENTORY_HEALTH] < 60) {    //if the bot has insufficient armor
...
bot_moveresult_t BotAttackMove(bot_state_t *bs, int tfl) {
  ...
  if (bs->weaponnum == WP_14 || bs->weaponnum == (WP_12 + WP_NUM_WEAPONS)) {//get real close
    attack_dist = 16;
    attack_range = 16;
  } else {
...
//					if (random() < selfpreservation) return;
  ...
  if (wi.flags & WFL_FIRERELEASED) {
    if (bs->flags
        & BFL_ATTACKED) {// check in here to either call the alt_fire or not!!  also, adjust the bots weapon...  hackery?
  ...
  } else { // check in here to either call the alt_fire or not!!  also, adjust the bots weapon...  hackery?
    if (bs->weaponnum > WP_NUM_WEAPONS) // it's an alt_fire!
    {
      bs->weaponnum -= WP_NUM_WEAPONS;
      trap_EA_Alt_Attack(bs->client);
...
void BotMapScripts(bot_state_t *bs) {
  ...
  if (!Q_stricmp(mapname, "q3tourney6")) {
    ...
    if (shootbutton) {
      ...
      bs->ideal_viewangles[YAW] = AngleMod(bs->ideal_viewangles[YAW]);
      //
      if (InFieldOfVision(bs->viewangles,
                          20,
                          bs->ideal_viewangles)) {// check in here to either call the alt_fire or not!!  also, adjust the bots weapon...  hackery?
        if (bs->weaponnum > WP_NUM_WEAPONS) // it's an alt_fire!
        {
          bs->weaponnum -= WP_NUM_WEAPONS;
          trap_EA_Alt_Attack(bs->client);
