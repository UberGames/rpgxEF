...
extern pclass_t ValueNameForClass( /*gentity_t *ent,*/ const char *s);
...
void G_Client_InitBodyQue(void) {
  ...
  if (bodyFadeSound == 0) {    // Initialize this sound.
...
static void CopyToBodyQue(gentity_t *ent) {
  ...
  switch (eState->legsAnim & ~ANIM_TOGGLEBIT) {
    case BOTH_DEATH1:
    case BOTH_DEAD1: eState->torsoAnim = eState->legsAnim = BOTH_DEAD1;
      break;
    case BOTH_DEATH2:
    case BOTH_DEAD2: eState->torsoAnim = eState->legsAnim = BOTH_DEAD2;
      break;
    default: eState->torsoAnim = eState->legsAnim = BOTH_DEAD1; //DEAD3
      break;
  }
...
static void ForceClientSkin(char *model, const char *skin) {
  ...
  if (!p || !p[0] || !p[1]) {
    ...
    if (!q || !q[0] || !q[1]) { //no slashes were found?? >.<
    ...
    for (i = 0; i < g_maxclients.integer; i++) {
      ...
      if (ent->client && ent->client->sess.sessionTeam == current_team) {
        ...
        if (x == current_skin_count) {
          ...
          Q_strncpyz(skinNamesAlreadyUsed[current_skin_count],
                     temp_model,
                     sizeof(skinNamesAlreadyUsed[current_skin_count]));
          current_skin_count++;
        }
      }
    }
...
void SetCSTeam(team_t team, char *teamname) {
  ...
  switch (team) {
    case TEAM_BLUE: trap_SetConfigstring(CS_BLUE_GROUP, teamname);
      break;
    case TEAM_RED: trap_SetConfigstring(CS_RED_GROUP, teamname);
      break;
...
void G_Client_UserinfoChanged(int clientNum) {
    ...
    for (i = 0; i < level.numConnectedClients; i++) {
      ...
      if (g_entities[i].client->ps.clientNum != client->ps.clientNum
        ...
        trap_SendServerCommand(ent - g_entities,
                               " print \"Unable to change name. A player already has that name on this server.\n\" ");
        changeName = qfalse;
        break;
      }
    }
  ...
  if (changeName) {
    ...
    if (pers->connected == CON_CONNECTED) {
      if (strcmp(oldname, pers->netname)) {
        if (!levelExiting && !level.intermissiontime) {//no need to do this during level changes
          ...
          if (reset) {
            if (!levelExiting) {//no need to do this during level changes
              trap_SendServerCommand(-1,
                                     va("print \"In-appropriate skin selected for %s on team %s\nSkin selection overridden from skin %s to skin %s\n\"",
                                        pers->netname,
                                        g_team_group_red.string,
  ...
  if (ent->r.svFlags & SVF_BOT) {
    s =
        va("n\\%s\\t\\%i\\p\\%i\\model\\%s\\hc\\%i\\w\\%i\\l\\%i\\skill\\%s\\age\\25\\height\\%s\\weight\\%s\\race\\Bot\\of\\%i\\admin\\0",
    ...
    s =
        va("n\\%s\\t\\%i\\p\\%i\\model\\%s\\hc\\%i\\w\\%i\\l\\%i\\age\\%s\\height\\%s\\weight\\%s\\race\\%s\\of\\%i\\admin\\%i",
           pers->netname, sess->sessionTeam, sess->sessionClass, model,
...
const char *G_Client_Connect(int clientNum, qboolean firstTime, qboolean isBot) {
  ...
  if (isBot) {
    ...
    ent->inuse = qtrue;
    if (!G_BotConnect(clientNum, static_cast<qboolean>(!firstTime))) {
      ...
      if (rpg_renamedPlayers.integer && !(ent->r.svFlags & SVF_BOT)) {
        ...
        for (i = 0; i < MAX_RECON_NAMES; i++) {
          ...
          if (!Q_stricmp(client->pers.ip, g_reconData[i].ipAddress)
            ...
            trap_SendServerCommand(-1,
                                   va("print \"%s" S_COLOR_WHITE " (With the previous name of %s" S_COLOR_WHITE ") connected\n\"",
                                      client->pers.netname,
                                      g_reconData[i].previousName));
            nameFound = qtrue;
            break;
          }
        }
...
void holoTent_think(gentity_t *ent) {
  ...
  switch (ent->count) {
    ...
    case 2: // desc1
      trap_SendServerCommand(ent - g_entities,
                             va("holo_data \"da%i\\%s\\\"", ent->health, holoData.desc1[ent->health]));
      break;
    case 3: // desc2
      trap_SendServerCommand(ent - g_entities,
                             va("holo_data \"db%i\\%s\\\"", ent->health, holoData.desc2[ent->health]));
      break;
    case 4: // image
...
void G_Client_Begin(int clientNum, qboolean careAboutWarmup, qboolean isBot, qboolean first) {
  ...
  if (client->sess.sessionTeam
      != TEAM_SPECTATOR /*&& g_holoIntro.integer==0 */ ) {    // Don't use transporter FX for spectators or those watching the holodoors.
    // send event
...
void G_Client_StoreClientInitialStatus(gentity_t *ent) {
  ...
  if (clientInitialStatus[ent->s.number].initialized) {//already set
    return;
  }

  if (ent->client->sess.sessionTeam == TEAM_SPECTATOR) {//don't store their data if they're just a spectator
    return;
  }

  trap_GetUserinfo(ent->s.number, userinfo, sizeof(userinfo));
  Q_strncpyz(clientInitialStatus[ent->s.number].model,
             Info_ValueForKey(userinfo, "model"),
             sizeof(clientInitialStatus[ent->s.number].model));
...
void G_RestoreClientInitialStatus(gentity_t *ent) {
  ...
  if (!clientInitialStatus[ent->s.number].initialized) {//not set
    return;
  }
  ...
  if (clientInitialStatus[ent->s.number].team != sess->sessionTeam &&
    ...
    SetClass(ent,
             ClassNameForValue(clientInitialStatus[ent->s.number].pClass),
             (char *) TeamName(clientInitialStatus[ent->s.number].team),
             qtrue);
  } else {
  ...
  if (Q_stricmp(clientInitialStatus[ent->s.number].model, Info_ValueForKey(userinfo, "model"))
      != 0) {//restore the model
...
void G_Client_Spawn(gentity_t *ent, int rpgx_spawn, qboolean fromDeath) {
  ...
  if (rpgx_spawn
      != 1) {//RPG-X: RedTechie - Make sure the spawn is regular spawn or spawn at current position (rpgx_spawn = current possition)
    if (client->sess.sessionTeam == TEAM_SPECTATOR) {
  ...
  if (oClass != client->sess.sessionClass) {//need to send the class change
  ...
  if (pClass != 0) {//no health boost on spawn for playerclasses
  ...
  if (ent->client->sess.sessionTeam == TEAM_SPECTATOR) {//spectators just get the title of the game
    ...
    if (!clientInitialStatus[ent->s.number].initialized) {//first time coming in
      switch (g_gametype.integer) {
        case GT_FFA:                // free for all
          trap_SendServerCommand(ent - g_entities, va("cp \"%s\"", rpg_welcomemessage.string));
          break;
...
void G_Client_Disconnect(int32_t clientNum) {
  ...
  if ((rpg_renamedPlayers.integer != 0) && ((ent->r.svFlags & SVF_BOT) == 0)) {
    ...
    if (foundName) {
      ...
      //Player Name
      Q_strncpyz(g_reconData[g_reconNum].previousName,
                 ent->client->pers.netname,
                 sizeof(g_reconData[g_reconNum].previousName));
...
void G_Client_CheckHealthInfoMessage(void) {
  ...
  for (i = 0, sendToCnt = 0; i < g_maxclients.integer; i++) {
    ...
    if (level.clients[i].pers.connected == CON_CONNECTED && level.clients[i].ps.stats[STAT_HEALTH] > 0
        &&//make sure they've actually spawned in already
            (level.clients[i].sess.sessionTeam == TEAM_SPECTATOR
                || g_classData[level.clients[i].sess.sessionClass].isMedical
                || g_classData[level.clients[i].sess.sessionClass].isAdmin)) {
      sendToClients[sendToCnt++] = i;
    }
  }

  if (!sendToCnt) {//no-one to send to
    return;
  }
  ...
  if (!cnt) {//no-one relevant changed health
    return;
  }
