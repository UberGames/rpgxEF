...
static void WP_FireHyperspanner(gentity_t *ent, qboolean alt_fire) {
	...
	classnames.append(&classnames, (void *) "func_breakable", LT_STRING, strlen("func_breakable")+1);
	classnames.append(&classnames, (void *) "misc_model_breakable", LT_STRING, strlen("misc_model_breakable")+1);
	...
	if(count) {
		...
		for(cont = validEnts.next(iter); cont != NULL; cont = validEnts.next(iter)) {
			e = static_cast<gentity_t *>(cont->data);
