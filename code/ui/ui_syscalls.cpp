...
qboolean trap_Key_IsDown( int keynum ) {
	return static_cast<qboolean>(syscall( UI_KEY_ISDOWN, keynum ));
}

qboolean trap_Key_GetOverstrikeMode( void ) {
	return static_cast<qboolean>(syscall( UI_KEY_GETOVERSTRIKEMODE ));
}
...
qboolean trap_SetCDKey( char *buf ) {
	return static_cast<qboolean>(syscall( UI_SET_CDKEY, buf ));
}
