...
void Atoms::SecurityCodeSetup() {
  ...
  if (!f) {
    UI_ConfirmMenu(menu_normal_text[MNT_ID_NOTTHERE], 0,
                   [](bool b) { SecurityFeedback(b); });
    return;
  }
  ...
  if (fileLen != SECURITY_SIZE) {
    UI_Logger(LL_ERROR, "rpgxid.dat is wrong size. %i, should be %i\n", fileLen,
              SECURITY_SIZE);
    UI_ConfirmMenu(menu_normal_text[MNT_ID_WRONGSIZE], 0,
                   [](bool b) { SecurityFeedback(b); });
    return;
  }
...
#endif
  {
    ...
    UI_ConfirmMenu(menu_normal_text[MNT_ID_INVALID], 0,
                   [](bool b) { SecurityFeedback(b); });
    return;
  }
  ...
  if (code->hash == atoul(sv_securityHash.string) &&
    ...
    while (code->hash == 0 || code->hash == SECURITY_HASH) {
      ...
      /*code->hash = (int32_t)(rand() / (((double)RAND_MAX + 1)/
       * SECURITY_HASH));*/
      for (i = 0; i < 32; i++) {
        if ((irandom(1, 2) - 1)) {
          bit += 1 << i;
...
std::string Atoms::LanguageFilename(std::string_view baseName,
                                    std::string_view baseExtension) {
  std::string result;
  char language[32];
  fileHandle_t file;
...
void Atoms::ParseButtonText() {
  ...
  while (buffer) {
    ...
    if (len) {
      ...
      if ((len == 1) && (token[0] == '/')) /* A NULL? */
      {
        ...
        menu_button_text[i][0] =
            (buffer - (len + 1)); /* The +1 is to get rid of the " at the
    ...
    if (uis.debug)
      UI_Logger(LL_DEBUG,
                "UI_ParseButtonText - Line: %i, String1: %s, String2: %s\n",
                i - 1, menu_button_text[i - 1][0], menu_button_text[i - 1][1]);

    if (i > MBT_MAX) {
      UI_Logger(LL_ERROR,
                "UI_ParseButtonText : too many values! Needed %d but got %d.\n",
                MBT_MAX, i);
      return;
    }
  }
  if (i != MBT_MAX) {
    UI_Logger(LL_DEBUG,
              "UI_ParseButtonText : not enough lines. Read %d of %d!\n", i,
              MBT_MAX);
    for (; i < MBT_MAX; i++) {
      menu_button_text[i][0] = "?";
      menu_button_text[i][1] = "?";
...
void Atoms::LoadMenuText() {
  ...
  if (!f) {
    Com_Error(ERR_FATAL,
              "UI_LoadMenuText : MP_NORMALTEXT.DAT file not found!\n");
    return;
  }

  if (len > MAXMENUTEXT) {
    Com_Error(ERR_FATAL,
              "UI_LoadMenuText : MP_NORMALTEXT.DAT size (%d) > max (%d)!\n",
              len, MAXMENUTEXT);
    return;
  }
  ...
  while (buffer) {
    ...
    if (uis.debug)
      UI_Logger(LL_DEBUG, "UI_ParseMenuText - Line: %i, String: %s\n", i - 1,
                menu_normal_text[i - 1]);

    if (i > MNT_MAX) {
      UI_Logger(LL_ERROR,
                "UI_ParseMenuText : too many values! Needed %d but got %d.\n",
                MNT_MAX, i);
      return;
    }
  }
...
void Atoms::MenuFrame2(menuframework_s *menu) {
  ...
  if (menu->titleI) {
    DrawProportionalString({menu->titleX, menu->titleY},
                           menu_normal_text[menu->titleI],
  ...
  trap_R_SetColor(colorTable[CT_DKBROWN1]);
  /*UI_DrawHandlePic(  30, 175,  47,  25, uis.whiteShader);*/ /* Mid left
                                                                 column*/
  DrawHandlePic({30, 392, 47, 33}, uis.whiteShader); /* Bottom left column */
  DrawHandlePic({30, 425, 128, 64},
                s_menuFrame.cornerLower); /* Bottom Left Corner */
  ...
  if (menu->footNoteEnum) {
    DrawProportionalString({MENU_TITLE_X, 440},
                           menu_normal_text[menu->footNoteEnum],
                           UI_RIGHT | UI_SMALLFONT, ColorTable[CT_LTORANGE]);
    MenuBottomLineEnd_Graphics(menu_normal_text[menu->footNoteEnum],
                               ColorTable[CT_LTBROWN1]);
  }
  trap_R_SetColor(nullptr);

  /* Print version */
  if (space)
    DrawProportionalString({371, 445}, Q3_VERSION, UI_TINYFONT,
                           ColorTable[CT_BLACK]);
}
...
bool Atoms::MenuBottomLineEnd_Graphics(string_view string, Color color) {
  ...
  DrawHandlePic({367, 438, holdLength, 18},
                uis.whiteShader); /* Bottom end line */

  /* TiM - stop the text leaving the box in some instances */
  return (holdLength < 0 ? -holdLength : holdLength) >=
         ProportionalStringWidth(Q3_VERSION, UI_TINYFONT);
}
...
void Atoms::FrameBottom_Graphics() {
  ...
  trap_R_SetColor(colorTable[CT_DKBROWN1]);
  DrawHandlePic({30, 173, 47, 27},
                uis.whiteShader); /* Top left column (81453) */
...
void Atoms::FrameTop_Graphics(menuframework_s *menu) {
  ...
  DrawHandlePic({30, 81, 47, 34},
                uis.whiteShader); /* Middle left hand column */
  ...
  DrawHandlePic(
      {152, 136, 135, 7},
      uis.whiteShader); /* 2nd line across bottom of top third section */

  trap_R_SetColor(colorTable[CT_LTPURPLE2]);
  DrawHandlePic(
      {290, 136, 12, 7},
      uis.whiteShader); /* 3rd line across bottom of top third section */

  trap_R_SetColor(colorTable[CT_LTBROWN1]); /* RPG-X ADDITION */
  DrawHandlePic(
      {305, 139, 60, 4},
      uis.whiteShader); /* 4th line across bottom of top third section */

  /*RPG-X REMOVE trap_R_SetColor( colorTable[CT_LTBROWN1]);*/
  DrawHandlePic(
      {368, 136, 111, 7},
      uis.whiteShader); /* 5th line across bottom of top third section */
...
void Atoms::MenuFrame_Cache() {
  s_menuFrame.cornerUpper =
      trap_R_RegisterShaderNoMip("menu/common/corner_ll_47_7.tga");
  s_menuFrame.cornerUpper2 =
      trap_R_RegisterShaderNoMip("menu/common/corner_ul_47_7.tga");
  s_menuFrame.cornerLower =
  ...
  for (i = 0; i < maxI; ++i) {
    if (menuGraphics[i].type == MG_GRAPHIC) {
      menuGraphics[i].graphic =
          trap_R_RegisterShaderNoMip(menuGraphics[i].file);
    }
  }
}
...
void Atoms::PrintMenuGraphics(menugraphics_s *menuGraphics, int32_t maxI) {
  ...
  for (i = 0; i < maxI; ++i) {
    if (menuGraphics[i].type == MG_GRAPHIC) {
      ...
      DrawHandlePic({menuGraphics[i].x, menuGraphics[i].y,
                     menuGraphics[i].width, menuGraphics[i].height},
                    menuGraphics[i].graphic);
      ...
      DrawProportionalString({menuGraphics[i].x, menuGraphics[i].y}, text,
                             menuGraphics[i].style,
                             ColorTable[menuGraphics[i].color]);
    } else if (menuGraphics[i].type == MG_NUMBER) {
      trap_R_SetColor(colorTable[menuGraphics[i].color]);
      DrawNumField({menuGraphics[i].x, menuGraphics[i].y}, menuGraphics[i].max,
                   menuGraphics[i].target, menuGraphics[i].width,
...
void Atoms::DrawNumField(const Point2dI &position, size_t width, int32_t value,
                         size_t charWidth, size_t) {
  if (width < 1) {
    return;
  }
...
bool Atoms::CursorInRect(const RectangleI &rect) {
  return !(uis.cursorx < rect.left || uis.cursory < rect.top ||
           uis.cursorx > rect.right || uis.cursory > rect.bottom);
}
...
void Atoms::Refresh(int32_t realtime) {
  ...
  if (uis.debug) {
    ...
    UI_DrawString(0, 0, va("(%d,%d)", uis.cursorx, uis.cursory),
                  UI_LEFT | UI_SMALLFONT, colorRed, qtrue);
    uis.widescreen.state = WIDESCREEN_CENTER;
  }
#endif
...
void Atoms::DrawRect(const common::RectangleF &rect, common::Color color) {
  trap_R_SetColor(color.values);

  auto rect_640 = Atoms::AdjustFrom640(rect);

  trap_R_DrawStretchPic(rect_640.left, rect_640.top, rect_640.width(), 1, 0, 0,
                        0, 0, uis.whiteShader);
  trap_R_DrawStretchPic(rect_640.left, rect_640.top, 1, rect_640.height(), 0, 0,
                        0, 0, uis.whiteShader);
  trap_R_DrawStretchPic(rect_640.left, rect_640.top + rect_640.height() - 1,
                        rect_640.width(), 1, 0, 0, 0, 0, uis.whiteShader);
  trap_R_DrawStretchPic(rect_640.left + rect_640.width() - 1, rect_640.top, 1,
                        rect_640.height(), 0, 0, 0, 0, uis.whiteShader);

  trap_R_SetColor(nullptr);
}

void Atoms::FillRect(const common::RectangleF &rect, common::Color color) {
  trap_R_SetColor(color.values);

  auto rect_640 = AdjustFrom640(rect);
  trap_R_DrawStretchPic(rect_640.left, rect_640.top, rect_640.width(),
                        rect_640.height(), 0, 0, 0, 0, uis.whiteShader);

  trap_R_SetColor(nullptr);
}

void Atoms::DrawHandlePic(const common::RectangleF &rect, qhandle_t hShader) {
  float s0;
  float s1;
  float t0;
  float t1;
  ...
  auto rect_640 = AdjustFrom640({rect.left, rect.top, w, h});
  trap_R_DrawStretchPic(rect.left, rect.left, rect.width(), rect.height(), s0,
                        t0, s1, t1, hShader);
}

void Atoms::DrawHandleStretchPic(const common::RectangleF &rect, float s0,
                                 float t0, float s1, float t1,
                                 qhandle_t hShader) {
  auto rect_640 = AdjustFrom640(rect);
  trap_R_DrawStretchPic(rect_640.left, rect_640.top, rect_640.width(),
                        rect_640.height(), s0, t0, s1, t1, hShader);
}

void Atoms::DrawNamedPic(const common::RectangleF &rect,
                         std::string_view picname) {
  auto hShader = trap_R_RegisterShaderNoMip(picname.data());
  auto rect_640 = AdjustFrom640(rect);
  trap_R_DrawStretchPic(rect_640.left, rect_640.top, rect_640.width(),
                        rect_640.height(), 0, 0, 1, 1, hShader);
}
...
void Atoms::AdjustFrom640(common::RectangleF &rect) {
  auto x = rect.left * uis.scalex;
  auto y = rect.top * uis.scaley;
  auto w = rect.width() * uis.scalex;
  auto h = rect.height() * uis.scaley;
...
common::RectangleI Atoms::AdjustFrom640(common::RectangleF rect) {
  auto x = rect.left * uis.scalex;
  auto y = rect.top * uis.scaley;
  auto w = rect.width() * uis.scalex;
  auto h = rect.height() * uis.scaley;
...
void Atoms::Init() {
  ...
  /*	uis.scale = uis.glconfig.vidHeight * (1.0/480.0);
      ...
      // wide screen
      uis.bias = 0.5 * ( uis.glconfig.vidWidth - ( uis.glconfig.vidHeight *
     (640.0/480.0) ) );
      }
      else {
      // no wide screen
  ...
  if (uis.glconfig.vidWidth * 480 > uis.glconfig.vidHeight * 640) {
    ...
    uis.widescreen.bias = 0.5 * (uis.glconfig.vidWidth -
                                 (uis.glconfig.vidHeight * (640.0 / 480.0)));
  } else {
  ...
  /* trap_Cvar_Create ("rpg_playIntro", "1", CVAR_ARCHIVE ); */ /*RPG-X | Phenix
                                                                   | 25/02/2005
                                                                 */
  UI_LogFuncEnd();
}
...
int32_t Atoms::GetClassSets() {
  ...
  numFiles = trap_FS_GetFileList("ext_data/classes", ".classes", fileBuffer,
                                 sizeof(fileBuffer));
...
int32_t Atoms::GetRankSets() {
  ...
  numFiles = trap_FS_GetFileList("ext_data/ranksets", ".ranks", fileBuffer,
                                 sizeof(fileBuffer));
  /*UI_Logger( LL_DEBUG, "%s\n", filePtr);*/
  ...
  while (i < numFiles) {
    ...
    Q_strncpyz(uis.rankSet.rankSetNames[i], filePtr,
               sizeof(uis.rankSet.rankSetNames[i]));
...
void Atoms::InitRanksData(char *ranksName) {
  ...
  Com_sprintf(filePath, sizeof(filePath), "ext_data/ranksets/%s.ranks",
              ranksName);
  ...
  if (!BG_ParseRankNames(filePath, uis.rankSet.rankNames,
    ...
    /* Rank attempt failed.  Try loading the defaults.  If we end up with no
     * loaded ranks... many menu elements will start crapping. bad */
    if (!BG_ParseRankNames(va("ext_data/ranksets/%s.ranks", RANKSET_DEFAULT),
                           uis.rankSet.rankNames,
                           sizeof(rankNames_t) * MAX_RANKS))
      trap_Error(va("UI_InitRanksData: Was unable to load default rankset: %s",
                    RANKSET_DEFAULT));
  } else {
    int32_t i;
    /*
     * nvm, all loaded good :)
     * set the current rank CVAR so it'll use this rankset next time they start
     * the game
     */
    trap_Cvar_Set("ui_currentRankSet", ranksName);

    /* eh... to be on the safe side, save the name of the ranks in a local
     * string */
    Q_strncpyz(uis.rankSet.rankSetName, ranksName,
               sizeof(uis.rankSet.rankSetName));

  refreshRank:
    /* using our current cvar'd rank, do a compare.  if we find a match, set our
     * player to that rank in the menu */
    for (i = 0, uis.currentRank = 0; i < MAX_RANKS; i++) {
      // TODO: replace UI_Cvar_VariableString by modern system call
      if (!Q_stricmp(uis.rankSet.rankNames[i].consoleName,
                     UI_Cvar_VariableString("ui_playerRank"))) {
        uis.currentRank = i;
        break;
      }
    }
...
int32_t Atoms::PopulateRanksArray(char **ranks) {
  ...
  ranks[i] = "Other";
  /*ranks[i+1] = 0; */ /*IMPORTANT: Spin controls need these or else values
                          bleed into different controls*/
...
int32_t Atoms::InitClassData(char *fileName) {
  ...
  Com_sprintf(filePath, sizeof(filePath), "ext_data/classes/%s.classes",
              fileName);
      ...
      while (1) {
        ...
        if (!Q_stricmpn(token, "formalName", 10)) {
          if (COM_ParseString(&textPtr, &token)) {
            UI_Logger(LL_ERROR,
                      "Error parsing formalName parameter in file: %s.\n",
                      fileName);
            continue;
          }
        ...
        if (!Q_stricmpn(token, "consoleName", 11)) {
          if (COM_ParseString(&textPtr, &token)) {
            UI_Logger(LL_ERROR,
                      "Error parsing consoleName parameter in file: %s.\n",
                      fileName);
            continue;
          }
...
bool Atoms::IsWidescreen() {
  return ui_handleWidescreen.integer && uis.widescreen.ratio &&
         uis.widescreen.state != WIDESCREEN_NONE;
}
...
void Atoms::LerpColor(common::Color a, common::Color b, common::Color &c,
                      float t) {
  int32_t i;
...
void Atoms::DrawBannerString2(const common::Point2dI &pos, std::string_view str,
                              common::Color color) {
  ...
  for (auto ch : str) {
    ch = ch & 255;
    if (ch == ' ') {
      ax += PROPB_SPACE_WIDTH + PROPB_SPACE_WIDTH * uis.scalex;
    } else {
      ch -= 'A';
      const auto ch_idx = static_cast<int32_t>(ch);
      const auto fcol = propMapB[ch][0] / 256.0f;
      const auto frow = propMapB[ch_idx][1] / 256.0f;
      const auto fwidth = propMapB[ch_idx][2] / 256.0f;
      const auto fheight = PROPB_HEIGHT / 256.0f;
      const auto aw = propMapB[ch_idx][2] * uis.scalex;
      const auto ah = PROPB_HEIGHT * uis.scaley;
      trap_R_DrawStretchPic(ax, ay, aw, ah, fcol, frow, fcol + fwidth,
                            frow + fheight, uis.charsetPropB);
      ax += aw + PROPB_GAP_WIDTH * uis.scalex;
    }
  }
...
void Atoms::DrawBannerString(Point2dI pos, std::string_view str, int32_t style,
                             common::Color color) {
  ...
  for (auto ch : str) {
    if (ch == ' ') {
      ...
      width += propMapB[ch - 'A'][2] + PROP_GAP_WIDTH;
    }
  }
...
size_t Atoms::ProportionalStringWidth(string_view str, int32_t style) {
  if (style == UI_TINYFONT) {
    auto width =
        std::accumulate(str.begin(), str.end(), 0U, [](auto sum, auto ch) {
...
void Atoms::DrawProportionalString2(common::Point2dI pos, std::string_view str,
                                    common::Color color, int32_t style,
                                    qhandle_t charset) {
  float ax;
  float ay, holdY;
  float aw;
  ...
  if (style & UI_TINYFONT) {
    for (auto it = str.begin(); it != str.end(); ++it) {
      ...
      if (ch == ' ') {
        ...
        ah = static_cast<float>(PROP_TINY_HEIGHT + special) * uis.scaley *
             sizeScale;
      ...
      ax +=
          aw + static_cast<float>(PROP_GAP_TINY_WIDTH) * uis.scalex * sizeScale;
    ...
    for (auto it = str.begin(); it != str.end(); ++it) {
      ...
      if (ch == ' ') {
        ...
        ah = static_cast<float>(PROP_BIG_HEIGHT + special) * uis.scaley *
             sizeScale;
      ...
      ax +=
          aw + static_cast<float>(PROP_GAP_BIG_WIDTH) * uis.scalex * sizeScale;

      if (IsWidescreen()) {
        ax -= PROP_GAP_BIG_WIDTH * uis.scalex * sizeScale *
              (1.0f - uis.widescreen.ratio);
      }
    }
  } else {
    for (auto it = str.begin(); it != str.end(); ++it) {
      ...
      if (IsWidescreen()) {
        ax -= PROP_GAP_WIDTH * uis.scalex * sizeScale *
              (1.0f - uis.widescreen.ratio);
      }
    }
  }
...
void Atoms::DrawProportionalString(common::Point2dI pos, std::string_view str,
                                   int32_t style, common::Color color) {
  Color drawcolor;
  size_t width;
  float sizeScale;
  int32_t charstyle = 0;
...
void Atoms::DrawString2(common::Point2dI pos, std::string_view str,
                        common::Color color, int32_t charw, int32_t charh) {
  ...
  for (auto it = str.begin(); it != str.end(); ++it) {
    ...
    if (ch == ' ') {
      ...
      trap_R_DrawStretchPic(ax, ay, aw, ah, fcol, frow, fcol + 0.03125,
                            frow + 0.0625, uis.charset);
    }
...
void Atoms::DrawString(common::Point2dI pos, std::string_view str,
                       int32_t style, common::Color color, bool highRes) {
  size_t len;
  int32_t charw;
  int32_t charh;
  Color newcolor;
...
void Atoms::DrawChar(common::Point2dI pos, char ch, int32_t style,
                     common::Color color) {
  std::string tmp{1, ch};
...
void Atoms::NeedCDAction(bool result) {
  if (!result) {
...
void Atoms::SetActiveMenu(uiMenuCommand_t menu) {
  /*
   * this should be the ONLY way the menu system is brought up, except for
   * ConsoleCommand below enusure minumum menu data is cached
   */
  Menu_Cache();
...
void Atoms::MouseEvent(const common::Point2dI &delta) {
  int32_t i;
  menucommon_s *m;
  ...
  for (i = 0; i < uis.activemenu->nitems; i++) {
    ...
    if (uis.activemenu->cursor != i) {
      ...
      ((menucommon_s *)(uis.activemenu->items[uis.activemenu->cursor_prev]))
          ->flags &= ~QMF_HASMOUSEFOCUS;

      if (!(((menucommon_s *)(uis.activemenu->items[uis.activemenu->cursor]))
                ->flags &
            QMF_SILENT)) {
    ...
    ((menucommon_s *)(uis.activemenu->items[uis.activemenu->cursor]))->flags |=
        QMF_HASMOUSEFOCUS;
    return;
  }
...
std::string Atoms::Argv(int32_t arg) {
  char buffer[MAX_STRING_CHARS];
  trap_Argv(arg, buffer, sizeof(buffer));
  return buffer;
}

std::string Atoms::CvarVariableString(std::string_view var_name) {
  char buffer[MAX_STRING_CHARS];
  trap_Cvar_VariableStringBuffer(var_name.data(), buffer, sizeof(buffer));
  return buffer;
}
...
bool Atoms::ConsoleCommand() {
  auto cmd = Argv(0);

  /* ensure minimum menu data is available*/
  Menu_Cache();

  if (common::icompare(cmd, "ui_cache")) {
    Cache();
    return true;
  }

  if (common::icompare(cmd, "ui_cdkey")) {
    UI_CDKeyMenu_f();
    return true;
  }

  if (common::icompare(cmd, "ui_emotes")) {
    UI_EmotesMenu(qtrue);
    return true;
  }

  if (common::icompare(cmd, "ui_admin")) {
    UI_AdminMenu(qtrue);
    return true;
  }

  if (common::icompare(cmd, "ui_turbolift")) {
    UI_TurboliftMenu(std::stoi(Argv(1)));
    return true;
  }

  if (common::icompare(cmd, "err_dialog")) {
    UI_ConfirmMenu(Argv(1), 0, 0);
    return true;
  }

  /* RPG-X | Marcin | 15/12/2008 */
  if (common::icompare(cmd, "ui_motd")) {
    UI_MotdMenu();
    return true;
  }

  if (common::icompare(cmd, "ui_motd_reset")) {
    MotdReset();
    return true;
  }
   ...
   */
  if (common::icompare(cmd, "ui_motd_line")) {
    MotdReceiveLine(Argv(1));
    return true;
  }

  if (common::icompare(cmd, "ui_msd")) {
    static char ui_msd1[MAX_STRING_CHARS], ui_msd2[MAX_STRING_CHARS],
        ui_msd3[MAX_STRING_CHARS], ui_msd4[MAX_STRING_CHARS],
        ui_msd5[MAX_STRING_CHARS], ui_msd6[MAX_STRING_CHARS],
        ui_msd7[MAX_STRING_CHARS], ui_msd8[MAX_STRING_CHARS],
        ui_msd9[MAX_STRING_CHARS], ui_msd10[MAX_STRING_CHARS];
    ...
    UI_msdMenu(atoi(ui_msd1), atoi(ui_msd2), atoi(ui_msd3), atoi(ui_msd4),
               atoi(ui_msd5), atoi(ui_msd6), atoi(ui_msd7), atoi(ui_msd8),
               atoi(ui_msd9), ui_msd10);
    return true;
  }

  if (common::icompare(cmd, "ui_transporter")) {
    UI_TransporterMenu(std::stoi(Argv(1)));
    return true;
  }

  if (common::icompare(cmd, "holo_data")) {
    /*HoloDataReceived(Argv(1));*/
    return true;
  }

  if (common::icompare(cmd, "ui_trdata")) {
    TransDataReceived(Argv(1).data());
    return true;
  }

  if (common::icompare(cmd, "ui_holodeck")) {
    /*UI_HolodeckMenu( atoi(Argv(1)) );*/
    return true;
  }

  if (common::icompare(cmd, "ui_sqlmenu")) {
    UI_sqlMenu();
    return true;
  }
    ...
    if (!Q_stricmp(token, "{")) {
      for (i = 0; i < CHARMAX; ++i) {
        ...
        if (!Q_stricmp(token, "{")) {
          ...
          trap_Print(va(
              S_COLOR_RED
              "ParseFontParms : Invalid FONTS.DAT data, near character %d!\n",
              i));
          return (NULL);
        }
