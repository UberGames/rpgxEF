...
=================
*/
static void UI_DrawBannerString2(int32_t x, int32_t y, const char *str,
                                 vec4_t color) {
  UI_LogFuncBegin();
  const char *s;
  char ch;
...
=================
*/
void UI_DrawBannerString(int32_t x, int32_t y, const char *str, int32_t style,
                         vec4_t color) {
  UI_LogFuncBegin();
  const char *s;
  int32_t ch;
...
static int32_t specialTinyPropChars[CHARMAX][2] = {
    ...
    {2, -3}, {0, -1}, {2, -3}, {2, -3}, /* 210 */
    {2, -3}, {3, -3}, {2, -3}, {2, -3}, {0, 0},  {0, -1},
    {2, -3}, {2, -3}, {2, -3}, {2, -3}, /* 220 */
    {2, -3}, {0, -1}, {0, -1}, {2, -3}, {2, -3}, {2, -3},
    {2, -3}, {2, -3}, {2, -3}, {0, 0}, /* 230 */
...
=================
*/
static void UI_DrawProportionalString2(int32_t x, int32_t y, const char *str,
                                       vec4_t color, int32_t style,
                                       qhandle_t charset) {
  UI_LogFuncBegin();
  const char *s;
  unsigned char ch;
  ...
  if (style & UI_TINYFONT) {
    ...
    while (*s) {
      ...
      if (UI_IsWidescreen())
        ax -= ((float)PROP_GAP_TINY_WIDTH * uis.scalex * sizeScale) *
    ...
    while (*s) {
      ...
      /* again adjust for widescreen */
      if (UI_IsWidescreen())
        ax -= ((float)PROP_GAP_BIG_WIDTH * uis.scalex * sizeScale) *
              (1.0f - uis.widescreen.ratio);
    ...
    while (*s) {
      ...
      /* again adjust for widescreen */
      if (UI_IsWidescreen())
        ax -= ((float)PROP_GAP_WIDTH * uis.scalex * sizeScale) *
              (1.0f - uis.widescreen.ratio);
...
=================
*/
void UI_DrawProportionalString(int32_t x, int32_t y, const char *str,
                               int32_t style, vec4_t color) {
  UI_LogFuncBegin();
  vec4_t drawcolor;
  int32_t width;
  float sizeScale;
  ...
  if (style & UI_TINYFONT) {
    UI_DrawProportionalString2(x, y, str, color, charstyle,
                               uis.charsetPropTiny);
  } else if (style & UI_BIGFONT) {
...
=================
*/
static void UI_DrawString2(int32_t x, int32_t y, const char *str, vec4_t color,
                           int32_t charw, int32_t charh) {
  UI_LogFuncBegin();
  const char *s;
  char ch;
  ...
  while (*s) {
    ...
    if (ch != ' ') {
      /*frow = (ch>>4)*0.0625;
      fcol = (ch&15)*0.0625;
      trap_R_DrawStretchPic( ax, ay, aw, ah, fcol, frow, fcol + 0.0625, frow +
      0.0625, uis.charset );*/
...
=================
*/
void UI_DrawString(int32_t x, int32_t y, const char *str, int32_t style,
                   vec4_t color, qboolean highRes) {
  UI_LogFuncBegin();
  int32_t len;
  int32_t charw;
  int32_t charh;
  ...
  if (style & UI_PULSE) {
    ...
    UI_LerpColor(color, lowlight, newcolor,
...
=================
*/
void UI_DrawChar(int32_t x, int32_t y, int32_t ch, int32_t style,
                 vec4_t color) {
  UI_LogFuncBegin();
  char buff[2];
...
void UI_SetActiveMenu(uiMenuCommand_t menu) {
   ...
   * UI_ConsoleCommand below enusure minumum menu data is cached
   */
  UI_LogFuncEnd();
  Menu_Cache();
    ...
    if (!Q_stricmp(token, "{")) {
      for (i = 0; i < CHARMAX; ++i) {
        ...
        if (!Q_stricmp(token, "{")) {
          ...
          trap_Print(va(S_COLOR_RED "UI_ParseFontParms : Invalid FONTS.DAT "
                                    "data, near character %d!\n",
                        i));
          UI_LogFuncEnd();
          return (NULL);
        }
 ...
 * Plus I don't want to delete it as I tore several layers of hair out coding it
 * rofl
 */
/*void	UI_InitRanksData( void ) {
        ...
        numFiles = trap_FS_GetFileList("ext_data/ranksets", ".ranks",
   fileBuffer, sizeof(fileBuffer) ); filePtr = fileBuffer;
        //UI_Logger( LL_DEBUG, "%s\n", filePtr);

        for ( i = 0, j = 0; i < numFiles && j < MAX_RANKSETS; i++,
   filePtr+=(int32_t)fileLen+1 ) { fileLen = strlen(filePtr);
        ...
        Com_sprintf( filePath, sizeof ( filePath ), "ext_data/ranksets/%s",
   filePtr );
...
void UI_DrawHandleStretchPic(float x, float y, float w, float h, float s0,
...
void UI_DrawRect(float x, float y, float width, float height,
  ...
  trap_R_DrawStretchPic(x, y + height - 1, width, 1, 0, 0, 0, 0,
                        uis.whiteShader);
  trap_R_DrawStretchPic(x + width - 1, y, 1, height, 0, 0, 0, 0,
                        uis.whiteShader);
...
qboolean UI_CursorInRect(int32_t x, int32_t y, int32_t width, int32_t height) {
  UI_LogFuncBegin();
  if (uis.cursorx < x || uis.cursory < y || uis.cursorx > x + width ||
...
==============
*/
static void UI_DrawNumField(int32_t x, int32_t y, int32_t width, int32_t value,
                            int32_t charWidth, int32_t charHeight) {
  UI_LogFuncBegin();
  char num[16], *ptr;
  int32_t l;
  int32_t frame;
...
void UI_PrintMenuGraphics(menugraphics_s *menuGraphics, int32_t maxI) {
  ...
  for (i = 0; i < maxI; ++i) {
    if (menuGraphics[i].type == MG_GRAPHIC) {
      ...
      UI_DrawHandlePic(menuGraphics[i].x, menuGraphics[i].y,
      ...
      UI_DrawProportionalString(menuGraphics[i].x, menuGraphics[i].y, text,
                                menuGraphics[i].style,
                                colorTable[menuGraphics[i].color]);
    } else if (menuGraphics[i].type == MG_NUMBER) {
      trap_R_SetColor(colorTable[menuGraphics[i].color]);
      UI_DrawNumField(menuGraphics[i].x, menuGraphics[i].y, menuGraphics[i].max,
...
static void UI_FrameTop_Graphics(menuframework_s *menu) {
  ...
  if (menu->titleI) {
    UI_DrawProportionalString(menu->titleX, menu->titleY,
...
static void UI_FrameBottom_Graphics(void) {
  ...
  UI_DrawHandlePic(30, 425, 128, 64,
...
=================
*/
void UI_MenuBottomLineEnd_Graphics(const char *string, int32_t color,
                                   qboolean *space) {
  UI_LogFuncBegin();
  ...
  UI_DrawHandlePic(367, 438, holdLength, 18,
                   uis.whiteShader); /* Bottom end line */

  /* TiM - stop the text leaving the box in some instances */
  if (space != NULL) {
    if ((holdLength < 0 ? -holdLength : holdLength) <
        UI_ProportionalStringWidth(Q3_VERSION, UI_TINYFONT))
      *space = qfalse;
    else
      *space = qtrue;
  }
...
void UI_MenuFrame(menuframework_s *menu) {
  ...
  if (menu->footNoteEnum) {
    ...
    UI_MenuBottomLineEnd_Graphics(menu_normal_text[menu->footNoteEnum],
                                  CT_LTBROWN1, &space);
  }
...
/*
UI_LanguageFilename - create a filename with an extension based on the value in
g_language
*/
void UI_LanguageFilename(char *baseName, char *baseExtension, char *finalName) {
  UI_LogFuncBegin();
  ...
  if (language[0] == '\0' || Q_stricmp("ENGLISH", language) == 0) {
    ...
    Com_sprintf(finalName, MAX_QPATH, "%s_%s.%s", baseName, language,
                baseExtension);
    ...
    {
      Com_sprintf(finalName, MAX_QPATH, "%s.%s", baseName,
                  baseExtension); /* the caller will give the error if this
                                     isn't there */
    } else {
