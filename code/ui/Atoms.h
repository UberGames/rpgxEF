...
public:
  ...
  static std::string LanguageFilename(std::string_view baseName,
  ...
  static bool MenuBottomLineEnd_Graphics(std::string_view string,
                                         common::Color color);
  ...
  static void DrawNumField(const common::Point2dI &position, size_t width,
                           int32_t value, size_t charWidth, size_t charHeight);

  static bool CursorInRect(const common::RectangleI &rect);

  static void Refresh(int32_t realtime);

  static void DrawRect(const common::RectangleF &rect, common::Color color);

  static void FillRect(const common::RectangleF &rect, common::Color color);

  static void DrawHandlePic(const common::RectangleF &rect, qhandle_t hShader);

  static void DrawHandleStretchPic(const common::RectangleF &rect, float s0,
                                   float t0, float s1, float t1,
                                   qhandle_t hShader);

  static void DrawNamedPic(const common::RectangleF &rect,
                           std::string_view picname);

  static void AdjustFrom640(float &x, float &y, float &w, float &h);

  static void AdjustFrom640(common::RectangleF &rect);

  static common::RectangleI AdjustFrom640(common::RectangleF rect);
  ...
  static void LerpColor(common::Color a, common::Color b, common::Color &c,
                        float t);

  static void DrawBannerString2(const common::Point2dI &pos,
                                std::string_view str, common::Color color);

  static void DrawBannerString(common::Point2dI pos,
                               std::string_view str, int32_t style,
                               common::Color color);

  static size_t ProportionalStringWidth(std::string_view str, int32_t style);

  static void DrawProportionalString2(common::Point2dI pos,
                                      std::string_view str, common::Color color,
                                      int32_t style, qhandle_t charset);

  static float ProportionalSizeScale(int32_t style);

  static void DrawProportionalString(common::Point2dI pos, std::string_view str,
                                     int32_t style, common::Color color);

  static void DrawString2(common::Point2dI pos, std::string_view str,
                          common::Color color, int32_t charw, int32_t charh);

  static void DrawString(common::Point2dI pos, std::string_view str,
                         int32_t style, common::Color color, bool highRes);

  static void DrawChar(common::Point2dI pos, char ch, int32_t style,
                       common::Color color);
  ...
  static void MouseEvent(const common::Point2dI& delta);

  static std::string Argv(int32_t arg);

  static std::string CvarVariableString(std::string_view var_name);
...
protected:
  static inline constexpr int32_t propMapB[26][3]{
      ...
      {156, 12, 21}, {183, 12, 21}, {207, 12, 32},

      {13, 55, 30},  {49, 55, 13},  {66, 55, 29},  {101, 55, 31},
      {135, 55, 21}, {158, 55, 40}, {204, 55, 32},

      {12, 97, 31},  {48, 97, 31},  {82, 97, 30},  {118, 97, 30},
      {153, 97, 30}, {185, 97, 25}, {213, 97, 30},

      {11, 139, 32}, {42, 139, 51}, {93, 139, 32}, {126, 139, 31},
      {158, 139, 25}};
