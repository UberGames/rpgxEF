...
static const int32_t INGAME_MENU_VERTICAL_SPACING = 24; // 28 // Marcin
...
static void IngamePlayer_DrawPlayer(void *self) {
  ...
  if (Q_stricmp(buf, s_ingame.playerModel)) {
    ...
    UI_PlayerInfo_SetInfo(&s_ingame.playerinfo, BOTH_STAND1, BOTH_STAND1,
                          viewangles, origin, WP_0,
                          trap_Cvar_VariableValue("height"),
                          trap_Cvar_VariableValue("weight"), qfalse);
  }

  b = &s_ingame.playermdl;
  UI_DrawPlayer(b->generic.x, b->generic.y, b->width, b->height, origin,
                &s_ingame.playerinfo, uis.realtime);
  UI_LogFuncEnd();
}
...
=================
*/
void InGame_LeaveAction(bool result) {
  if (!result) {
    return;
  }
...
void InGame_Event(void *ptr, int32_t notification) {
  ...
  case ID_RESTART:
    UI_ConfirmMenu(menu_normal_text[MNT_RESTART_MATCH], 0,
                   InGame_RestartAction);
    break;
...
static void UI_InGameMenu_Draw(void) {
  ...
  // UI_DrawHandlePic( 482, 136,  MENU_BUTTON_MED_WIDTH - 22,
  // MENU_BUTTON_MED_HEIGHT, uis.whiteShader);
  UI_DrawHandlePic(460 + MENU_BUTTON_MED_WIDTH - 6, 136, -19,
                   MENU_BUTTON_MED_HEIGHT, uis.graphicButtonLeftEnd); // right
  ...
  UI_DrawHandlePic(s_ingame.removebots.generic.x - 14,
                   s_ingame.removebots.generic.y, MENU_BUTTON_MED_HEIGHT,
                   MENU_BUTTON_MED_HEIGHT, uis.graphicButtonLeftEnd);

  trap_R_SetColor(colorTable[CT_DKPURPLE1]);
  //	UI_DrawHandlePic(s_ingame.teamorders.generic.x - 14,
  // s_ingame.teamorders.generic.y, 		MENU_BUTTON_MED_HEIGHT,
  // MENU_BUTTON_MED_HEIGHT, uis.graphicButtonLeftEnd);
  ...
  UI_DrawHandlePic(s_ingame.restart.generic.x - 14, s_ingame.restart.generic.y,
                   MENU_BUTTON_MED_HEIGHT, MENU_BUTTON_MED_HEIGHT + 6,
                   uis.graphicButtonLeftEnd);
  ...
  trap_R_SetColor(colorTable[CT_DKPURPLE1]);
  UI_DrawHandlePic(s_ingame.spectate.generic.x - 14,
                   s_ingame.spectate.generic.y, MENU_BUTTON_MED_HEIGHT,
                   MENU_BUTTON_MED_HEIGHT + 6, uis.graphicButtonLeftEnd);

  UI_DrawHandlePic(s_ingame.join.generic.x - 14, s_ingame.join.generic.y,
                   MENU_BUTTON_MED_HEIGHT, MENU_BUTTON_MED_HEIGHT + 6,
                   uis.graphicButtonLeftEnd);

  UI_DrawProportionalString(74, 66, "15567", UI_RIGHT | UI_TINYFONT,
                            colorTable[CT_BLACK]);
  UI_DrawProportionalString(74, 84, "2439", UI_RIGHT | UI_TINYFONT,
                            colorTable[CT_BLACK]);
  UI_DrawProportionalString(74, 188, "3814", UI_RIGHT | UI_TINYFONT,
                            colorTable[CT_BLACK]);
  UI_DrawProportionalString(74, 206, "4800", UI_RIGHT | UI_TINYFONT,
                            colorTable[CT_BLACK]);
  UI_DrawProportionalString(74, 395, "5671-1", UI_RIGHT | UI_TINYFONT,
                            colorTable[CT_BLACK]);

  // UI_DrawProportionalString( 584, 142, "1219",UI_RIGHT|UI_TINYFONT,
  // colorTable[CT_BLACK]);
  ...
  trap_R_SetColor(colorTable[CT_DKPURPLE2]);
  UI_DrawHandlePic(230, 158, -16, 16,
                   uis.graphicBracket1CornerLU);      // 375	//RU -200
  ...
  UI_DrawHandlePic(230, 418, -16, -16,
                   uis.graphicBracket1CornerLU); // 375	//RD

  UI_DrawProportionalString(271, 165, menu_normal_text[MNT_PLAYERSTATS],
                            UI_SMALLFONT, colorTable[CT_LTGOLD1]);

  // ghey hack to stop Com_sprintf whining if we overflow the buffer lol
  Com_sprintf(string, sizeof(string), "%s: %-25.25s",
              menu_normal_text[MNT_NAME], UI_Cvar_VariableString("name"));
  // Q_strncpyz( string, string, 25 );
  UI_DrawProportionalString(270, y, string, UI_LEFT | UI_SMALLFONT,
                            colorTable[CT_LTGOLD1]);

  y += yOffset;
  Com_sprintf(string, sizeof(string), "%s: %-25.25s",
              menu_normal_text[MNT_CLASS], playerClass);
  // Q_strncpyz( string, string, 25 );
  UI_DrawProportionalString(270, y, string, UI_LEFT | UI_SMALLFONT,
                            colorTable[CT_LTGOLD1]);
  ...
  Com_sprintf(string, sizeof(string), "%s: %3.2f%s",
              menu_normal_text[MNT_HEIGHT], scale, HEIGHT_UNIT);
  // Q_strncpyz( string, string, 25 );
  UI_DrawProportionalString(432, y, string, UI_LEFT | UI_SMALLFONT,
                            colorTable[CT_LTGOLD1]);

  y += yOffset;
  Com_sprintf(string, sizeof(string), "%s: %-25.25s",
              menu_normal_text[MNT_RANK],
              uis.rankSet.rankNames[uis.currentRank].formalName);
  // Q_strncpyz( string, string, 25 );
  UI_DrawProportionalString(270, y, string, UI_LEFT | UI_SMALLFONT,
                            colorTable[CT_LTGOLD1]);

  scale = trap_Cvar_VariableValue("height") *
          trap_Cvar_VariableValue("weight") * (float)BASE_WEIGHT;
  if (s_ingame.playerinfo.gender == GENDER_FEMALE)
    scale *= (float)FEMALE_OFFSET;

  Com_sprintf(string, sizeof(string), "%s: %3.2f%s",
              menu_normal_text[MNT_WEIGHT], scale, WEIGHT_UNIT);
  // Q_strncpyz( string, string, 25 );
  UI_DrawProportionalString(432, y, string, UI_LEFT | UI_SMALLFONT,
                            colorTable[CT_LTGOLD1]);

  y += yOffset;
  Com_sprintf(string, sizeof(string), "%s: %-25.25s", menu_normal_text[MNT_AGE],
              UI_Cvar_VariableString("age"));
  // Q_strncpyz( string, string, 25 );
  UI_DrawProportionalString(270, y, string, UI_LEFT | UI_SMALLFONT,
                            colorTable[CT_LTGOLD1]);

  Com_sprintf(string, sizeof(string), "%s: %-25.25s",
              menu_normal_text[MNT_RACE], UI_Cvar_VariableString("race"));
  // Q_strncpyz( string, string, 25 );
  UI_DrawProportionalString(432, y, string, UI_LEFT | UI_SMALLFONT,
                            colorTable[CT_LTGOLD1]);

  //	UI_DrawProportionalString(	319,  193,
  //UI_Cvar_VariableString("name"), UI_LEFT|UI_SMALLFONT,
  // colorTable[CT_LTGOLD1]); 	UI_DrawProportionalString( 319,  215, pClass,
  // UI_LEFT|UI_SMALLFONT, colorTable[CT_LTGOLD1]); 	UI_DrawProportionalString(
  //319,  236, pRank, UI_LEFT|UI_SMALLFONT, colorTable[CT_LTGOLD1]);
  // UI_DrawProportionalString(  319,  257, pModel, UI_LEFT|UI_SMALLFONT,
  // colorTable[CT_LTGOLD1]);

  UI_DrawProportionalString(271, 309, menu_normal_text[MNT_PLAYERSTATUS],
                            UI_SMALLFONT, colorTable[CT_LTGOLD1]);
  ...
  UI_DrawHandlePic(
      (3 + 271 +
       (UI_ProportionalStringWidth(menu_normal_text[MNT_PLAYERSTATS],
                                   UI_SMALLFONT))),
      165,
      ((313 - (UI_ProportionalStringWidth(menu_normal_text[MNT_PLAYERSTATS],
                                          UI_SMALLFONT))) -
       3),
      18, uis.whiteShader);
  ...
  UI_DrawHandlePic(
      (3 + 271 +
       (UI_ProportionalStringWidth(menu_normal_text[MNT_PLAYERSTATUS],
                                   UI_SMALLFONT))),
      309,
      ((144 - (UI_ProportionalStringWidth(menu_normal_text[MNT_PLAYERSTATUS],
                                          UI_SMALLFONT))) -
       3),
      18, uis.whiteShader);
  ...
  // dark blue
  trap_R_SetColor(colorTable[CT_DKBROWN1]);
  UI_DrawHandlePic(404, 284, 47, 18,
                   uis.whiteShader); // main bars along the bottom //257
  ...
  UI_DrawHandlePic(460 + MENU_BUTTON_MED_WIDTH - 6, 309, -19, 18,
                   uis.graphicButtonLeftEnd);
  ...
  // text
  UI_DrawProportionalString(570, 314, "4516", UI_TINYFONT,
                            colorTable[CT_BLACK]);
...
void InGame_MenuInit(void) {
  ...
  // s_ingame.screenshot.width						=
  // MENU_BUTTON_MED_WIDTH;
...
void InGame_Cache(void) {
  UI_LogFuncBegin();
  s_ingame.graphic_12_8_LU =
      trap_R_RegisterShaderNoMip("menu/common/corner_ul_8_12.tga");
  s_ingame.graphic_12_8_LL =
      trap_R_RegisterShaderNoMip("menu/common/corner_ll_8_12.tga");
  s_ingame.graphic_12_8_RU =
      trap_R_RegisterShaderNoMip("menu/common/corner_ur_8_12.tga");
  s_ingame.graphic_12_8_RL =
      trap_R_RegisterShaderNoMip("menu/common/corner_lr_8_12.tga");
}
...
static void UI_InGameQuitMenu_Draw(void) {
  ...
  // Rounded button that takes place of INGAME MENU button
  trap_R_SetColor(colorTable[CT_LTBROWN1]);
  UI_DrawHandlePic(482, 136, MENU_BUTTON_MED_WIDTH - 22, MENU_BUTTON_MED_HEIGHT,
                   uis.whiteShader);
  UI_DrawHandlePic(460 + MENU_BUTTON_MED_WIDTH - 6, 136, -19,
                   MENU_BUTTON_MED_HEIGHT, uis.graphicButtonLeftEnd); // right
